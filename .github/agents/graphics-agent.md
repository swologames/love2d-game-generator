...
- Examples of mandatory splits:
  ...
  - Never put particle logic AND shader logic AND camera logic in one file

### Required File Architecture Pattern
```
src/systems/
  ParticleManager.lua     -- <80 lines: manages emitter instances
  ShaderSystem.lua        -- shader load + uniform control
  CameraSystem.lua        -- camera follow, zoom, bounds
  ScreenShake.lua         -- shake offset calculation
src/shaders/
  crt.glsl                -- CRT effect only
  bloom.glsl              -- bloom only
  explosion.glsl          -- explosion distortion only
```
